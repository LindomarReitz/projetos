...
public class MidiaDAO {
	...
	public void salvar(Midia midia) {
		try {
			...
			System.out.println("Não foi possível inserir a mídia. Erro: " + e.getMessage());
		} finally {
			try{ 
	...
	public void atualizar(Midia midia) {
		try {
			...
			System.out.println("Não foi possível alterar a mídia. Erro: " + e.getMessage());
		} finally {
			try{ 
				if (this.sessao.isOpen()) {
					this.sessao.close();
	...
	public void excluir(Midia midia) {
		try {
			...
			System.out.println("Não foi possível excluir a mídia. Erro: " + e.getMessage());
		} finally {
			try{ 
				if (this.sessao.isOpen()) {
					this.sessao.close();
	...
	public Midia buscarMidia(int codigo) {
		Midia midia = null;
		try {
			this.sessao = HibernateUtil.getSessionFactory().getCurrentSession();
			this.transacao = this.sessao.beginTransaction();
